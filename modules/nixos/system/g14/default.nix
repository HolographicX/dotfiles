...
with lib.custom; let
  ...
  g14_patches = fetchGit {
    ...
    ref = "6.9";
    rev = "0331ce0e41373c8dda30fe6ecb01e059e30da313";
  };
in {
    ...
    boot.kernelPackages = let
      linux_custom_pkg = { fetchurl, buildLinux, ... } @ args:
        ...
        buildLinux (args // rec {
          ...
          src = fetchurl {
            url = "https://cdn.kernel.org/pub/linux/kernel/v6.x/linux-6.9.1.tar.xz";
            # Replace this with the correct SHA-256 hash after the first build.
            hash = "sha256-AbQUupj9GJ7NVEQ1yvOGCuKnkOPsSPWqcP30LcTFwEo=";
          };
          kernelPatches = map (patch: { inherit patch; }) [
            ...
            "${g14_patches}/0001-ALSA-PCI-HDA-Adjust-G814JZR-to-use-SPI-init-for-amp.patch"
            "${g14_patches}/0002-hid-asus-use-hid-for-brightness-control-on-keyboard.patch"
            "${g14_patches}/0003-Debugging.patch"
            "${g14_patches}/0004-asus-wmi-don-t-error-out-if-platform_profile-already.patch"
            "${g14_patches}/0005-hid-asus-add-USB_DEVICE_ID_ASUSTEK_DUO_KEYBOARD.patch"
